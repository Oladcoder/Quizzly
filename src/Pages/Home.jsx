...
const {theme, setTheme} = useContext(themeContext)
  ...
  return (
    <div className={`flex justify-center ${theme ? 'bg-gray-800': ''} items-center flex-col bubbles-background`}>
      ...
      <p className={`font-gambarino ${theme ? 'text-white shadow-lg': ''} font-telma text-2xl`}> Knowledge Unlocked, One Question at a Time. </p>
      <Link to='options'>
      <button className={`bg-gray-800 ${theme ? ' text-gray-700 shadow-md shadow-gray-700' : ''} hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-md mt-4 shadow-lg hover:shadow-xl transition duration-300 ease-in-out`}>Start Quiz</button>
      </Link>
